    ...
    C: Circuit<E> + Send,
{
    ...
    let (start, mut provers, _, _) =
        THREAD_POOL.install(|| create_proof_batch_priority_inner(circuits))?;

    let (h_params, l_params, a_inputs_source, a_aux_source, b_g1_inputs_source, b_g1_aux_source, b_g2_inputs_source, b_g2_aux_source, assignments) =
    THREAD_POOL.install(|| {
        // get params
        ...
        (h_params, l_params, a_inputs_source, a_aux_source, b_g1_inputs_source, b_g1_aux_source, b_g2_inputs_source, b_g2_aux_source, assignments)
    });
    ...
    let input_len = assignments[0].0.len();
    ...
    C: Circuit<E> + Send,
{
    ...
    */
    ...
    Ok((start, provers, Vec::new(), Vec::new()))
}
